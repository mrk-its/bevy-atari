...
use bevy::ecs::{ReadOnlyFetch, Resources, World, WorldQuery};
...
impl<Q: WorldQuery> PassNode<Q> {
    pub fn new(descriptor: PassDescriptor, pipeline_name: Option<String>) -> Self {
            ...
            &mut |render_pass| {
                for camera_info in self.cameras.iter() {
                    ...
                    for visible_entity in visible_entities.iter() {
                        ...
                        let mut wait_for_next_pipeline = false;
                        for render_command in draw.render_commands.iter() {
                            if let RenderCommand::SetPipeline{..} = render_command {
                                wait_for_next_pipeline = false;
                            } else {
                                if wait_for_next_pipeline {
                                    continue
                                }
                            }
                            match render_command {
                                RenderCommand::SetPipeline { pipeline } => {
                                    ...
                                    if descriptor.name != self.pipeline_name {
                                        wait_for_next_pipeline = true;
                                        continue;
                                    }
